...
module.exports = {
    createConfiguration : async(req, res)=>{
        ...
        if(configurationDetails && configurationDetails!==false){
            ...
            return res.status(401).json({"status": 404,"success": false,"message": "Can't Create OTP Details. Try again!"});
        }
    },
    verifyOtp: async (req,res)=>{
        let err,configuration_data;
        [err,configuration_data] = await to(mailConfigurationService.verifyotpDetails(req.body));
        ...
        if(configuration_data && configuration_data!==false){
            ...
            return res.status(404).json({"status": 404,"success": false,"message": "Entered OTP invalid/expired. Try again!"});
        }
    },
